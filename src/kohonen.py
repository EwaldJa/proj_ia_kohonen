...
class Neuron:
  ...
  def learn(self,eta,sigma,posxbmu,posybmu,x):
    ...
    diffPosBest = numpy.linalg.norm( numpy.array([ self .posx, self .posy]) - numpy.array([posxbmu, posybmu]) )
    self.weights[:] = self.weights[:] + eta * ( x - self.weights ) * numpy.exp( - ( numpy.power(diffPosBest, 2) / (2 * numpy.math.pow(sigma, 2) ) ) )
